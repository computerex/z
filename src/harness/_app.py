...
def main():
    ...
    if args.freeze_safe:
        ...
        # The runnable code lives in this file's package directory
        # (src/harness/ in a dev checkout, site-packages/harness/ after install).
        ...
        # Copy the entire harness package → SAFE_MODE_DIR/src/harness/
        dst_harness = SAFE_MODE_DIR / "src" / "harness"
        shutil.copytree(pkg_dir, dst_harness, ignore=shutil.ignore_patterns("__pycache__", "*.pyc"))
        con.print(f"  [green]✓[/green] Copied harness package ({len(list(dst_harness.rglob('*.py')))} files)")

        # Write a runnable shim at SAFE_MODE_DIR/harness.py so the safe-mode
        # launcher below can still invoke `python harness.py`.
        shim = (
            "#!/usr/bin/env python3\n"
        ...
        (SAFE_MODE_DIR / "harness.py").write_text(shim, encoding="utf-8")
        con.print(f"  [green]✓[/green] Wrote harness.py shim")
