...
from .config import Config, _merge_non_empty, get_global_config_path, load_json_config, save_json_config
...
async def run_single(
    ...
    if _schema and not error_info:
        from .output_protocol import validate_against_schema, fill_schema_gaps
...
def main():
    ...
    if args.freeze_safe:
        ...
        # Get the harness package directory (where main.py lives)
        harness_pkg = Path(__file__).parent.absolute()
        # Project root is 2 levels up from src/harness/
        project_root = harness_pkg.parent.parent
        ...
        # Copy the harness package to SAFE_MODE_DIR/harness/
        dst_harness = SAFE_MODE_DIR / "harness"
        shutil.copytree(harness_pkg, dst_harness)
        con.print(f"  [green]✓[/green] Copied harness package ({len(list(dst_harness.rglob('*.py')))} files)")
        
        # Create a minimal runner script
        runner = SAFE_MODE_DIR / "run.py"
        runner.write_text('''"""Safe mode runner."""
import sys, os
...
if __name__ == '__main__':
    sys.exit(main() or 0)
''')
        con.print(f"  [green]✓[/green] Created safe mode runner")
    ...
    if args.safe_mode:
        ...
        safe_runner = SAFE_MODE_DIR / "run.py"
        
        if not safe_runner.exists():
            con = Console()
            con.print("\n  [red]✗[/red] Safe mode not found!")
            con.print(f"  [dim]Expected at:[/dim] {safe_runner}")
        ...
        # Add safe mode dir to path
        env = os.environ.copy()
        env["PYTHONPATH"] = str(SAFE_MODE_DIR) + os.pathsep + env.get("PYTHONPATH", "")
        ...
        result = subprocess.run(
            [sys.executable, str(safe_runner)] + other_args,
            env=env,
            cwd=os.getcwd(),
        )
    ...
    if args.telegram:
        try:
            ...
            from .remote.manager import RemoteInputManager
            from .remote.telegram import TelegramProvider
        except ImportError:
