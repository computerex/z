...
func getSystemPrompt(workspace string) string {
	return fmt.Sprintf(defaultSystemPrompt,
		workspace, workspace, workspace, workspace, workspace, workspace, workspace, workspace, workspace,
		goOS, goArch, shellName, homeDir, workspace)
}
...
Parameters:
- path: (required) The path of the directory to list contents for (relative to the current working directory %s)
...
Parameters:
- path: (required) The path of the directory to search in (relative to the current working directory %s). This directory will be recursively searched.
...
Parameters:
- path: (required) The path to the image file (relative to the current working directory %s). Supports jpg, png, gif, webp formats.
...
- Do not ask for more information than necessary. Use the tools provided to accomplish the user's request efficiently and effectively. When you've completed your task, you must use the attempt_completion tool to present the result of the user.
...
4. After receiving tool results, summarize key findings before proceeding. Don't just silently chain tool calls - communicate insights.
